use crate::{
    ActivityKey, BusinessProcessModelAndNotation, EbiObject, Importable,
    ebi_objects::business_process_model_and_notation::{
        BPMNElement, BPMNProcess, MessageFlow, SequenceFlow,
    },
    traits::importable::{ImporterParameter, ImporterParameterValues, from_string},
};
use anyhow::{Context, Result, anyhow};
use itertools::Itertools;
use quick_xml::{
    NsReader,
    events::{BytesEnd, BytesStart, Event},
    name::{Namespace, ResolveResult},
};
use std::{
    collections::{HashMap, hash_map::Entry},
    io::BufRead,
};

impl Importable for BusinessProcessModelAndNotation {
    const IMPORTER_PARAMETERS: &[ImporterParameter] = &[];

    const FILE_FORMAT_SPECIFICATION_LATEX: &str = "A Business Process Model and Notation (BPMN) model follows the OMG 2.0.2 standard~\\cite{omg2011bpmn}.
    Currently, a sub-set of elements is supported.";

    fn import_as_object(
        reader: &mut dyn BufRead,
        parameter_values: &ImporterParameterValues,
    ) -> Result<EbiObject> {
        Ok(EbiObject::BusinessProcessModelAndNotation(Self::import(
            reader,
            parameter_values,
        )?))
    }

    fn import(
        reader: &mut dyn std::io::BufRead,
        _parameter_values: &crate::traits::importable::ImporterParameterValues,
    ) -> Result<Self>
    where
        Self: Sized,
    {
        let mut xml_reader = NsReader::from_reader(reader);
        xml_reader.config_mut().trim_text(true);

        let mut buf = vec![];
        let mut state = State::new();
        loop {
            buf.clear();
            let (namespace, xml_event) = xml_reader
                .read_resolved_event_into(&mut buf)
                .with_context(|| "cannot read XML event")?;
            let in_namespace = is_in_namespace(namespace);
            match (in_namespace, xml_event) {
                //start tag
                (true, Event::Start(e)) => {
                    println!(
                        "start tag {}",
                        String::from_utf8_lossy(e.local_name().as_ref()),
                    );
                    state.open_tag(&e).with_context(|| {
                        format!(
                            "start tag `{}` at position {}",
                            String::from_utf8_lossy(e.local_name().as_ref()),
                            xml_reader.buffer_position()
                        )
                    })?;
                }

                //end of tag
                (true, Event::End(e)) => state.close_tag(&e).with_context(|| {
                    format!(
                        "close tag `{}` at position {}",
                        String::from_utf8_lossy(e.local_name().as_ref()),
                        xml_reader.buffer_position()
                    )
                })?,

                //empty tag
                (true, Event::Empty(e)) => state.empty_tag(&e).with_context(|| {
                    format!(
                        "empty tag `{}` at position {}",
                        String::from_utf8_lossy(e.local_name().as_ref()),
                        xml_reader.buffer_position()
                    )
                })?,

                //end of file: return the tree if we can finish
                (_, Event::Eof) => {
                    state.can_eof().with_context(|| "unexpected end of file")?;
                    return Ok(state.to_model()?);
                }

                _ => (),
            }
        }
    }
}
from_string!(BusinessProcessModelAndNotation);

#[derive(Clone, Copy, Debug)]
enum BPMNElementType {
    StartEvent,
    EndEvent,
    Task,
    ExclusiveGateway,
    InclusiveGateway,
}

impl BPMNElementType {
    fn parse(e: &BytesStart, state: &State) -> Option<Self> {
        if state.open_tags.len() >= 2
            && state.open_tags[state.open_tags.len() - 2..]
                == [b"definitions".to_vec(), b"process".to_vec()]
        {
            match e.local_name().as_ref() {
                b"startEvent" => Some(BPMNElementType::StartEvent),
                b"endEvent" => Some(BPMNElementType::EndEvent),
                b"task" => Some(BPMNElementType::Task),
                b"exclusiveGateway" => Some(BPMNElementType::ExclusiveGateway),
                b"inclusiveGateway" => Some(BPMNElementType::InclusiveGateway),
                _ => None,
            }
        } else {
            None
        }
    }

    fn parse_to_element(self, e: &BytesStart, state: &mut State) -> Result<BPMNElement> {
        Ok(match self {
            BPMNElementType::StartEvent => BPMNElement::StartEvent,
            BPMNElementType::EndEvent => BPMNElement::EndEvent,
            BPMNElementType::Task => {
                if let Some(label) = parse_attribute(e, "name") {
                    let activity = state.activity_key.process_activity(&label);
                    BPMNElement::Task { activity }
                } else {
                    return Err(anyhow!("task must have a name"));
                }
            }
            BPMNElementType::ExclusiveGateway => BPMNElement::ExclusiveGateway,
            BPMNElementType::InclusiveGateway => BPMNElement::InclusiveGateway,
        })
    }
}

#[derive(Clone, Debug)]
pub struct DraftSequenceFlow {}

impl DraftSequenceFlow {
    fn parse(e: &BytesStart, state: &State) -> Option<()> {
        if state.open_tags.len() >= 2
            && state.open_tags[state.open_tags.len() - 2..]
                == [b"definitions".to_vec(), b"process".to_vec()]
        {
            match e.local_name().as_ref() {
                b"sequenceFlow" => Some(()),
                _ => None,
            }
        } else {
            None
        }
    }

    fn parse_to_sequence_flow(e: &BytesStart, state: &mut State) -> Result<SequenceFlow> {
        //parse source and add if necessary
        let new_index = state.element_id_2_index.len();
        let source_ref = parse_attribute(e, "sourceRef")
            .with_context(|| "flow must have an attribute `sourceRef`")?;
        let source_element_index =
            *state
                .element_id_2_index
                .entry(source_ref)
                .or_insert_with(|| {
                    state.element_index_2_element.push(None);
                    new_index
                });

        //parse target and add if necessary
        let new_index = state.element_id_2_index.len();
        let target_ref = parse_attribute(e, "sourceRef")
            .with_context(|| "flow must have an attribute `sourceRef`")?;
        let target_element_index =
            *state
                .element_id_2_index
                .entry(target_ref)
                .or_insert_with(|| {
                    state.element_index_2_element.push(None);
                    new_index
                });

        Ok(SequenceFlow {
            source_element_index,
            target_element_index,
        })
    }
}

struct DraftMessageFlow {
    id: String,
    source_id: String,
    target_id: String,
}

impl DraftMessageFlow {
    fn parse(e: &BytesStart, state: &State) -> Option<()> {
        if state.open_tags.len() >= 2
            && state.open_tags[state.open_tags.len() - 2..]
                == [b"definitions".to_vec(), b"collaboration".to_vec()]
        {
            match e.local_name().as_ref() {
                b"messageFlow" => Some(()),
                _ => None,
            }
        } else {
            None
        }
    }

    fn parse_to_message_flow(e: &BytesStart, _state: &mut State) -> Result<DraftMessageFlow> {
        //parse id
        let id = parse_attribute(e, "id").with_context(|| "message flow must have an id")?;

        //parse source
        let source_id = parse_attribute(e, "sourceRef")
            .with_context(|| "message flow must have an attribute `sourceRef`")?;

        //parse target
        let target_id = parse_attribute(e, "targetRef")
            .with_context(|| "flow must have an attribute `targetRef`")?;

        Ok(DraftMessageFlow {
            source_id,
            target_id,
            id,
        })
    }
}

struct State {
    activity_key: ActivityKey,
    open_tags: Vec<Vec<u8>>,
    in_definitions: bool,
    in_process: bool,
    closed_processes: Vec<BPMNProcess>,

    //draft process
    element_id_2_index: HashMap<String, usize>,
    element_index_2_element: Vec<Option<BPMNElement>>,
    flow_id_2_index: HashMap<String, usize>,
    flow_index_2_flow: Vec<SequenceFlow>,

    //draft messages
    messages: Vec<DraftMessageFlow>,
}

impl State {
    fn new() -> Self {
        Self {
            activity_key: ActivityKey::new(),
            open_tags: vec![],
            in_definitions: false,
            in_process: false,
            closed_processes: vec![],
            element_id_2_index: HashMap::new(),
            element_index_2_element: vec![],
            flow_id_2_index: HashMap::new(),
            flow_index_2_flow: vec![],
            messages: vec![],
        }
    }

    fn empty_tag(&mut self, e: &BytesStart) -> Result<()> {
        self.open_tag(e)?;
        self.close_tag(&e.to_end())
    }

    fn open_tag(&mut self, e: &BytesStart) -> Result<()> {
        match (
            e.local_name().as_ref(),
            BPMNElementType::parse(&e, self),
            DraftSequenceFlow::parse(e, self),
            DraftMessageFlow::parse(e, self),
        ) {
            (b"definitions", _, _, _) => {
                if self.in_definitions {
                    return Err(anyhow!("cannot nest `definitions` tags"));
                } else {
                    self.in_definitions = true;
                }
            }
            (b"process", _, _, _) => {
                self.open_process().with_context(|| "open process")?;
            }
            //element tag
            (_, Some(element_type), _, _) => {
                self.open_element(element_type, &e).with_context(|| {
                    anyhow!("parsing element tag `{:?}`", e.local_name().as_ref())
                })?;
            }
            //sequence flow tag
            (_, _, Some(()), _) => {
                self.open_sequence_flow(&e).with_context(|| {
                    anyhow!("parsing tag sequence flow `{:?}`", e.local_name().as_ref())
                })?;
            }
            // message flow tag
            (_, _, _, Some(())) => {
                self.open_message_flow(&e).with_context(|| {
                    anyhow!("parsing tag message flow `{:?}`", e.local_name().as_ref())
                })?;
            }
            _ => {}
        };

        self.open_tags.push(e.local_name().as_ref().to_owned());

        // println!("{}", debug(&self.open_tags));

        Ok(())
    }

    fn close_tag(&mut self, e: &BytesEnd) -> Result<()> {
        if let Some(most_recent_open_tag) = self.open_tags.pop() {
            if most_recent_open_tag == e.local_name().as_ref() {
                //closing tag matches last remaining opening tag

                match e.local_name().as_ref() {
                    b"process" => {
                        self.close_process().with_context(|| "close process")?;
                    }
                    _ => {}
                };
                Ok(())
            } else {
                Err(anyhow!(
                    "attempted to close tag `{}` but `{}` was open",
                    String::from_utf8_lossy(e.local_name().as_ref()),
                    String::from_utf8_lossy(&most_recent_open_tag)
                ))
            }
        } else {
            Err(anyhow!(
                "attempted to close tag `{}` that was not open",
                String::from_utf8_lossy(e.local_name().as_ref())
            ))
        }
    }

    fn can_eof(&self) -> Result<()> {
        if let Some(tag) = self.open_tags.iter().next() {
            Err(anyhow!(
                "file ended while tag `{}` was still open",
                String::from_utf8_lossy(&tag)
            ))
        } else {
            Ok(())
        }
    }

    fn open_element(&mut self, element_type: BPMNElementType, e: &BytesStart) -> Result<()> {
        println!("\topen element {:?}", element_type);

        //create the element
        let element = element_type.parse_to_element(e, self)?;

        //read the id
        if let Some(id) = parse_attribute(e, "id") {
            // println!("\t\tread id `{}`", id);
            let new_index = self.element_id_2_index.len();
            match self.element_id_2_index.entry(id.clone()) {
                Entry::Occupied(oe) => {
                    if let Some(_) = self.element_index_2_element[*oe.get()] {
                        return Err(anyhow!("two elements have the id `{}`", id));
                    } else {
                        //replace the None with the element
                        self.element_index_2_element[*oe.get()] = Some(element);
                    }
                }
                Entry::Vacant(vacant_entry) => {
                    self.element_index_2_element.push(Some(element));
                    vacant_entry.insert(new_index);
                }
            }
        } else {
            return Err(anyhow!("element must have an id"));
        }

        // println!("{:?}", self.element_index_2_element);

        Ok(())
    }

    fn open_message_flow(&mut self, e: &BytesStart) -> Result<()> {
        println!("\topen message flow");

        let flow = DraftMessageFlow::parse_to_message_flow(e, self)?;
        self.messages.push(flow);

        Ok(())
    }

    fn open_sequence_flow(&mut self, e: &BytesStart) -> Result<()> {
        println!("\topen sequence flow");

        let flow = DraftSequenceFlow::parse_to_sequence_flow(e, self)?;

        //read the id
        if let Some(id) = parse_attribute(e, "id") {
            let new_index = self.flow_index_2_flow.len();
            match self.flow_id_2_index.entry(id.clone()) {
                Entry::Occupied(_) => {
                    return Err(anyhow!("sequence flow with id `{}` is declared twice", id));
                }
                Entry::Vacant(vacant_entry) => {
                    vacant_entry.insert(new_index);
                    self.flow_index_2_flow.push(flow);
                }
            }
        } else {
            return Err(anyhow!("sequence flow must have an id"));
        }

        Ok(())
    }

    fn open_process(&mut self) -> Result<()> {
        if self.in_process {
            return Err(anyhow!("cannot nest `process` tags"));
        }

        self.in_process = true;

        Ok(())
    }

    fn close_process(&mut self) -> Result<()> {
        println!("\tclose process");
        self.in_process = false;

        let mut element_id_2_index = HashMap::new();
        let mut element_index_2_element = vec![];
        let mut flow_id_2_index = HashMap::new();
        let mut flow_index_2_flow = vec![];

        std::mem::swap(&mut self.element_id_2_index, &mut element_id_2_index);
        std::mem::swap(
            &mut self.element_index_2_element,
            &mut element_index_2_element,
        );
        std::mem::swap(&mut self.flow_id_2_index, &mut flow_id_2_index);
        std::mem::swap(&mut self.flow_index_2_flow, &mut flow_index_2_flow);

        //check whether all mentioned elements have been actually declared
        let mut element_index_2_element_x = Vec::with_capacity(element_index_2_element.len());
        for (index, element) in element_index_2_element.into_iter().enumerate() {
            if let Some(element) = element {
                element_index_2_element_x.push(element);
            } else {
                //None value found
                if let Some((id, _)) = element_id_2_index
                    .iter()
                    .find(|(_, indexx)| **indexx == index)
                {
                    return Err(anyhow!(
                        "id `{}` was mentioned but not declared as an element",
                        id
                    ));
                } else {
                    return Err(anyhow!(
                        "an id was mentioned but not declared as an element"
                    ));
                }
            }
        }

        self.closed_processes.push(BPMNProcess {
            element_id_2_index,
            element_index_2_element: element_index_2_element_x,
            flow_id_2_index,
            flow_index_2_flow,
        });

        Ok(())
    }

    fn to_model(self) -> Result<BusinessProcessModelAndNotation> {
        if self.closed_processes.len() > 0 {
            //connect the messages
            let mut messages = Vec::with_capacity(self.messages.len());
            {
                for draft_message in &self.messages {
                    let (source_pool, source_element) = self
                        .find_element_with_id(&draft_message.source_id)
                        .ok_or_else(|| {
                            anyhow!(
                                "id `{}` mentioned but not declared",
                                draft_message.source_id
                            )
                        })?;
                    let (target_pool, target_element) = self
                        .find_element_with_id(&draft_message.target_id)
                        .ok_or_else(|| {
                            anyhow!(
                                "id `{}` mentioned but not declared",
                                draft_message.source_id
                            )
                        })?;
                    messages.push(MessageFlow {
                        id: draft_message.id.clone(),
                        source_pool,
                        source_element,
                        target_pool,
                        target_element,
                    })
                }
            }

            //construct result
            let result = BusinessProcessModelAndNotation {
                activity_key: self.activity_key,
                processes: self.closed_processes,
                message_flows: messages,
            };

            //check structural correctness
            result
                .is_structurally_correct()
                .with_context(|| "model is not structurally correct")?;
            Ok(result)
        } else {
            Err(anyhow!("no process found"))
        }
    }

    fn find_element_with_id(&self, id: &str) -> Option<(usize, usize)> {
        for (pool_index, process) in self.closed_processes.iter().enumerate() {
            if let Some(element_index) = process.element_id_2_index.get(id) {
                return Some((pool_index, *element_index));
            }
        }
        None
    }
}

fn debug(inn: &Vec<Vec<u8>>) -> String {
    inn.iter()
        .map(|s| String::from_utf8_lossy(s).to_string())
        .join(", ")
}

fn is_in_namespace(result: ResolveResult) -> bool {
    match result {
        ResolveResult::Unbound => true,
        ResolveResult::Bound(Namespace(b"http://www.omg.org/spec/BPMN/20100524/MODEL")) => true,
        _ => false,
    }
}

fn parse_attribute(e: &BytesStart, attribute_name: &str) -> Option<String> {
    if let Ok(Some(attribute)) = e.try_get_attribute(attribute_name) {
        Some(
            String::from_utf8_lossy(&attribute.value)
                .as_ref()
                .to_owned(),
        )
    } else {
        None
    }
}

impl BusinessProcessModelAndNotation {
    pub fn is_structurally_correct(&self) -> Result<()> {
        //each message goes between pools
        for message_flow in &self.message_flows {
            if message_flow.source_pool == message_flow.target_pool {
                return Err(anyhow!(
                    "message flow with id `{}` is intra-pool",
                    message_flow.id
                ));
            }
        }

        Ok(())
    }
}

#[cfg(test)]
mod tests {
    use crate::BusinessProcessModelAndNotation;
    use std::fs::{self};

    #[test]
    fn bpmn_import() {
        let fin = fs::read_to_string("testfiles/model.bpmn").unwrap();
        let log = fin.parse::<BusinessProcessModelAndNotation>().unwrap();

        assert_eq!(log.processes[0].flow_id_2_index.len(), 10);
    }

    #[test]
    fn bpmn_pool_import() {
        let fin = fs::read_to_string("testfiles/model-pool.bpmn").unwrap();
        let log = fin.parse::<BusinessProcessModelAndNotation>().unwrap();

        assert_eq!(log.processes.len(), 2);
        assert_eq!(log.processes[0].flow_id_2_index.len(), 2);
        assert_eq!(log.message_flows.len(), 1);
    }
}
