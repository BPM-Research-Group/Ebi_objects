...
impl Exportable for ProcessTreeMarkupLanguage {
    fn export_from_object(object: EbiObject, f: &mut dyn Write) -> Result<()> {
        match object {
            EbiObject::ProcessTree(tree) => Self { tree }.export(f),
            EbiObject::StochasticProcessTree(stree) => Self { tree: stree.into() }.export(f),
            _ => Err(anyhow!(
                "cannot export {} {} as PTML",
...
impl Graphable for ProcessTreeMarkupLanguage {
