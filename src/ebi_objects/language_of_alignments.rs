...
use crate::{
    constants::ebi_object::EbiObject, ebi_objects::labelled_petri_net::TransitionIndex, line_reader::LineReader, Activity, ActivityKey, ActivityKeyTranslator, Exportable, Importable, Infoable, TranslateActivityKey
};
...
impl Exportable for LanguageOfAlignments {
    fn export_from_object(object: EbiObject, f: &mut dyn std::io::Write) -> Result<()> {
        match object {
            EbiObject::LanguageOfAlignments(alignments) => alignments.export(f),
            EbiObject::StochasticLanguageOfAlignments(sali) => Into::<Self>::into(sali).export(f),
