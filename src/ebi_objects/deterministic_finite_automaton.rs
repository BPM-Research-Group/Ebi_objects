...
impl Exportable for DeterministicFiniteAutomaton {
    fn export_from_object(object: EbiObject, f: &mut dyn std::io::Write) -> Result<()> {
        match object {
            EbiObject::DeterministicFiniteAutomaton(dfa) => dfa.export(f),
            ...
            EbiObject::DirectlyFollowsModel(dfm) => Into::<Self>::into(dfm).export(f),
            EbiObject::DirectlyFollowsGraph(dfg) => Into::<Self>::into(dfg).export(f),
...
impl Graphable for DeterministicFiniteAutomaton {
