use crate::{BusinessProcessModelAndNotation, bpmn::objects::BPMNObject};
...
impl BusinessProcessModelAndNotation {
    pub fn is_structurally_correct(&self) -> Result<()> {
        ...
        for message_flow in &self.message_flows {
            ...
            if message_flow.source_process_rank == message_flow.target_process_rank {
                return Err(anyhow!(
            ...
            if let Some(obj) = self.processes[message_flow.source_process_rank]
                .find_object_with_index(message_flow.source_element_index)
            {
            ...
            if let Some(obj) = self.processes[message_flow.target_process_rank]
                .find_object_with_index(message_flow.target_element_index)
            {
